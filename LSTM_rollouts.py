...
class LSTMModel(nn.Module):
    ...
    def forward(self, x_seq, u_seq):
        ...
        """
        seq = x_seq#torch.cat([x_seq, u_seq], dim=2)  # (batch_size, seq_len, n_x+n_u)
...
if __name__ == '__main__':
    ...
    x = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_x.npy')[:,:10000*200]).float().to(device)
    u = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_u.npy')[:,:10000*200]).float().to(device)
    y = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_y.npy')[:,:10000*200]).float().to(device)
    x[0, :] = x[0, :] - torch.pi * torch.ones(x[0, :].shape, device=x.device)
    y[0, :] = y[0, :] - torch.pi * torch.ones(y[0, :].shape, device=y.device)
    
    # Load testing data
    x_test = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_x.npy')[:,10000*200:11000*200]).float().to(device)
    u_test = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_u.npy')[:,10000*200:11000*200]).float().to(device)
    y_test = torch.from_numpy(np.load('data/undamped_pendulum_10ic_large_y.npy')[:,10000*200:11000*200]).float().to(device)
    ...
    # --------------------------
    seq_length = 35  # initial window length
    ...
    # --------------------------
    lstm_model = torch.load('models/LSTM_pendulum_ic10_model_undamped.pt').to(device)
    ...
    num_epochs = 20#50  # adjust as needed
    initial_teacher_ratio = 1.0
    final_teacher_ratio = 0.0
    
    # For CSV logging: create a list to store per-epoch values.
    # csv_rows = []
    ...
    '''with open('epoch_loss_10.csv', 'w', newline='') as f:
        ...
        for epoch in range(num_epochs):
            lstm_model.train()
            epoch_loss = 0.0
            teacher_ratio = initial_teacher_ratio - (initial_teacher_ratio - final_teacher_ratio) * (epoch / num_epochs)
            ...
            for batch in train_loader:
                x_batch, u_batch, y_batch = batch  # Shapes: (B, seq_length, n_x), (B, seq_length+rollout_steps, n_u), (B, rollout_steps, n_x)
                optimizer.zero_grad()
                loss = rollout_loss(lstm_model, x_batch, u_batch, y_batch, teacher_ratio)
                loss.backward()
                optimizer.step()
            ...
            with torch.no_grad():
                for batch in test_loader:
                    ...
                    loss_val = rollout_loss(lstm_model, x_batch, u_batch, y_batch, teacher_ratio)
                    test_loss += loss_val.item() * x_batch.size(0)
            avg_test_loss = test_loss / len(test_dataset)
            
            if epoch % 10 == 0:
                print(f"Epoch {epoch}: Rollout Loss = {avg_loss:.6f}, Test Loss = {avg_test_loss:.6f}, Teacher Ratio = {teacher_ratio:.3f}")
            ...
            writer.writerow([epoch, avg_loss, avg_test_loss, teacher_ratio])
            f.flush()
    ...
    plt.title('LSTM: Ground Truth vs. Predicted Trajectory (Trajectory 1)')
    plt.legend()
    plt.savefig('plots/LSTM_10deg_roll_longer_3.png')
    ...
    plt.xlabel('Time step')
    plt.ylabel('Pendulum angle (degree)')
    plt.title('LSTM: Ground Truth vs. Predicted Trajectory (Trajectory 2)')
    plt.legend()
